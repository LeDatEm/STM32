...
uint16_t count = 0;
bool lastbtnInc = true;
bool lastbtnDec = true;
/* USER CODE END PV */
...
int main(void)
{
  ...
  while (1)
  {
	  bool btnInc = (GPIOB->IDR & GPIO_PIN_15) == 0;
	  bool btnDec = (GPIOB->IDR & GPIO_PIN_14) == 0;

	  count = (btnInc && !lastbtnInc) ? (count + 1) % 10000 : count;
	  count = (btnDec && !lastbtnDec) ? (count -1 + 10000) % 10000 : count;
...
/* USER CODE BEGIN 4 */
void scanLED(int tx){
	uint8_t LED_PINS[] = {GPIO_PIN_0, GPIO_PIN_1, GPIO_PIN_2, GPIO_PIN_3};
	for(int i = 0; i < 4; ++i){
